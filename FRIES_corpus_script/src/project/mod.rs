...
use crate::command::{self, Sanitizer};
#[macro_use]
mod template;
mod util;
...
use self::util::{find_this_package_pathbuf, get_project_toml_value, Manifest};
...
impl FuzzProject {
    ...
    pub fn create_from_real() -> Result<Self> {
        let project_dir_path = find_this_package_pathbuf();
    ...
    fn get_cargo_subcommand_with_buildoptions(
        &self,
        sub_command: &str,
    ...
    fn cargo_run(&self, build_options: &BuildOptions, target: &str) -> Result<process::Command> {
        let mut cmd = self.get_cargo_subcommand_with_buildoptions("run", build_options)?;
