...
pub fn find_this_package_pathbuf() -> PathBuf {
    let current_dir_path = env::current_dir().unwrap();
    let cargo_toml_path = current_dir_path.join("Cargo.toml");
    ...
    match fs::File::open(&cargo_toml_path) {
        Ok(mut f) => {
            ...
            f.read_to_end(&mut data)
                .with_context(|| format!("failed to read {} toml file", cargo_toml_path.display()))
                .unwrap(); //懒惰求值

            let _: toml::Value = toml::from_slice(&data)
                .with_context(|| {
...
impl Manifest {
    /// 根据清单所在文件夹来读取清单，创建Manifest
    pub fn create(project_dir_path: PathBuf) -> Result<Self> {
        let toml_value = get_project_toml_value(project_dir_path.clone())?;
            ...
            .and_then(toml::Value::as_str)
        {
            Some(s) => s.to_string(),
            None => {
                panic!("faied to find crate_name in {}", project_dir_path.display());
            }
        };
