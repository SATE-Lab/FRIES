...
impl FuzzProject {
    ...
    pub fn create_from_real() -> Result<Self> {
        ...
        let mut fuzz_dir_path;
        //let mut toml_value;
        for entry in fs::read_dir(project_dir_path)? {
            fuzz_dir_path = entry?.path();
            let toml_value = get_project_toml_value(fuzz_dir_path)?;
        ...
        let target_dir_path = fuzz_dir_path.to_owned().join(DEFAULT_FUZZ_TARGET_DIR_NAME);
