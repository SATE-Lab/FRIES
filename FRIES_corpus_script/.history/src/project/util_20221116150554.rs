...
pub fn find_this_package() -> Result<PathBuf> {
    ...
    loop {
        ...
        match fs::File::open(&cargo_toml_path) {
            ...
            Ok(mut f){
                data.clear();
                f.read_to_end(&mut data).with_context(||format!("failed to read {}", cargo_toml_path.display()));
                let value: toml::Value = toml::from_slice(&data).with_context(||format!("failed to decode the file at {}",cargo_toml_path.display()));
            }
        }
    }
    bail!(
