...
use crate::command::subcmd::{FuzzDirWrapper, Init};
...
impl FuzzProject {
    ...
    pub fn init(init: &mut Init) -> Result<Self> {
        ...
        let fuzz_cargo_toml = fs::File::create(fuzz_cargo_toml.to_owned()).with_context(|| {
            format!(
        ...
        fuzz_cargo_toml.write_fmt(cargo_toml_template!(fuzz_manifest.));
