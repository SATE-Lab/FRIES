...
fn parse_dependent_manifest(lib_name: &str, dependent_path: PathBuf) {
    parse_package_manifest(lib_name, dependent_path);
}

fn parse_package_manifest(lib_name: &str, dependent_path: PathBuf) {
    let manifest = cargo_toml::Manifest::from_path(dependent_path.to_owned()).unwrap();
    match manifest.package {
        Some(package) => {
            println!("This dependent has a root package. Start to parse [package.workspace].");
            match package.workspace {
                Some(package_workspace) => {
                    // 这种情况说明package存在于某个根workspace
                    // TODO: 解析dependent，看看有没有lib_name
                }
                None => match manifest.workspace {
                    Some(workspace) => {
                        ...
                        for member in workspace.members {
                            ...
                            parse_package_manifest(lib_name, member_path);
                        }
                    }
                    None => {
