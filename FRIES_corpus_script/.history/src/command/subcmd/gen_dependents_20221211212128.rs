...
impl RunCommand for GenDependents {
    fn run_command(&mut self) -> Result<(), anyhow::Error> {
        clone_dependents_repositories(&self.lib_name, 3);
        get_valid_dependent_crates(&self.lib_name)?;
        Ok(())
    }
}
...
/// 参数：1.名字 2.依赖最大数目
/// 返回值：String向量
fn get_crate_dependents_repositories(name: &str, max_dependent_num: i32) -> Result<Vec<String>> {
    println!(
...
fn get_valid_dependent_crates(lib_name: &str) -> anyhow::Result<()> {
    let dependents_dir_path = get_dependents_dir_path();
    for entry in fs::read_dir(dependents_dir_path.to_owned())? {
        let entry_path = entry?.path();
        let entry_path_str = entry_path.to_str().with_context(|| {
            format!(
                "Failed to get the entry of directory [{}]",
                dependents_dir_path.display()
            )
        })?;
        println!("Find entry in directory dependents : {}", entry_path_str);

        let split = entry_path_str.split('/');
        let entry_name_str = split.last().unwrap();
        let mut split = entry_name_str.split('_'); //找到待测package的名字
        let dependency_lib_name = split.next().unwrap();
        //println!("{}, {}", entry_name_str, dependency_lib_name);
        ...
        parse_dependent_manifest(lib_name, entry_path);
    }

    Ok(())
}

fn parse_dependent_manifest(lib_name: &str, dependent_path: PathBuf) -> Result<(), Error> {
    let manifest = cargo_toml::Manifest::from_path(dependent_path).with_context(|| {
        format!(
            "Failed to read root manifest in {}",
