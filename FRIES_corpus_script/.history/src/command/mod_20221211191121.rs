...
enum Command {
    ...
    /// Get dependent from Crates.io, and parse out valid packages
    Getdep(subcmd::GenDependents),
...
impl RunCommand for Command {
    ...
    fn run_command(&mut self) -> Result<()> {
        match self {
            ...
            Command::Getdep(x) => x.run_command(),
        }
    }
}
...
impl fmt::Display for BuildOptions {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if self.triple != current_platform::CURRENT_PLATFORM {
...
mod manifest_util {
    ...
    pub fn get_project_toml_value(dir_path: PathBuf) -> anyhow::Result<toml::Value> {
        ...
        match fs::File::open(&cargo_toml_path) {
            ...
            Err(_) => {
                panic!("failed to read {} toml file", cargo_toml_path.display());
            }
        }
    }
}
