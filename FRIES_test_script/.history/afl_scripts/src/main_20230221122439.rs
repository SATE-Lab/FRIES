...
fn init_afl_input(crate_name: &str) {
    ...
    for test in &tests {
        ...
        for afl_file in &afl_files {
            ...
            let mut cmd = Command::new(replay_target_path.as_os_str()).arg(afl_file.as_os_str());
            let exit_status = cmd.stdout(Stdio::null()).stderr(Stdio::null());

            println!("exit_status: {:?}", exit_status);
            let exit_status = exit_status.status().unwrap();
            if exit_status.success() {
