...
lazy_static! {
    static ref CRATE_SRC_DIR: HashMap<&'static str, String> = {
        ...
        m.insert("url", "/Users/yxz/workspace/fuzz/experiment/rust-url/".to_string());
        /*
...
lazy_static! {
    ...
    static ref CRATE_TEST_DIR: HashMap<&'static str, &'static str> = {
        let mut m = HashMap::new();
        m.insert("url", "/Users/yxz/workspace/fuzz/experiment/rust-url/fuzz_dir/url_afl_work");
        /*
...
lazy_static! {
    static ref CRATE_PATCH_PATH: HashMap<&'static str, &'static str> = {
        let mut m = HashMap::new();
        m.insert("url", "{path=\"//Users/yxz/workspace/fuzz/experiment/rust-url/\"}");
...
pub fn check_pre_condition(crate_name: &str) -> Vec<String> {
    check_static();
    let crate_test_dir_path = PathBuf::from(CRATE_TEST_DIR.get(crate_name).unwrap());
    let afl_init_dir_path = crate_test_dir_path.join(AFL_INPUT_DIR);

    check_no_empty_directory(&afl_init_dir_path);

    // 检查afl_files文件夹中的文件
    // 比如 /Users/yxz/workspace/fuzz/experiment/rust-url/fuzz_dir/url_afl_work/afl_files
    let test_file_dir = crate_test_dir_path.join(TEST_FILE_DIR);
    ...
    // 检查replay_files文件夹中的文件
    let replay_file_dir = crate_test_dir_path.join(REPLAY_FILE_DIR);
...
fn init_test_dir(crate_name: &str, tests: &[String]) {
    ...
    //生成输出目录
    let output_dir = test_dir_path.join(AFL_OUTPUT_DIR);
    ...
    //创建cargo.toml文件
    let cargo_toml_path = test_dir_path.join(CARGO_TOML);
    ...
    //对于每个test_file新建项目
    for test in tests {
        let test_cargo_path = test_dir_path.clone().join(test);
    ...
    for test in tests {
        ...
        let replay_cargo_path = test_dir_path.clone().join(&replay);
        replays.push(replay);
        let _ = Command::new("cargo")
            .arg("new")
    ...
    let build_script_path = test_dir_path.join(BUILD_SCRIPT);
    ...
    let build_script = build_script_content(&test_dir_path);
    build_script_file
    ...
    //为每个test crate添加依赖
    for test in tests {
        let test_cargo_toml_path = test_dir_path.clone().join(test).join(CARGO_TOML);
        let mut file = OpenOptions::new().read(true).open(test_cargo_toml_path).unwrap();
        let mut buf;
    ...
    for replay in &replays {
        let replay_cargo_toml_path = test_dir_path.clone().join(replay).join(CARGO_TOML);
        let mut file = OpenOptions::new()
            .append(true)
    ...
    for test_file_name in tests {
        ...
        // afl_files里面的文件
        // test_url13.rs
        let src_path = test_dir_path
            .clone()
            .join(TEST_FILE_DIR)
            .join(test_file_name.clone() + ".rs");

        // 目标文件：
        // 每个项目文件里的.../test_url13/src/main.rs
        let dst_path = test_dir_path
            .clone()
            .join(test_file_name)
            .join("src")
            .join("main.rs");
    ...
    //复制replay文件
    for replay_file_name in &replays {
        // 复制起始文件：
        // replay_files里面的文件
        // replay_url13.rs
        let src_path = test_dir_path
            .clone()
            .join(REPLAY_FILE_DIR)
            .join(replay_file_name.clone() + ".rs");
...
fn build_afl_tests(crate_name: &str) {
    ...
    Command::new("cargo")
        ...
        .arg("build")
        .arg("--offline")
        .current_dir(test_dir_path.as_os_str())
        .output()
        .unwrap();
}
...
fn init_afl_input(crate_name: &str) {
    ...
    for test in &tests {
        ...
        for afl_input in &afl_inputs {
            //println!("{}", afl_file.display());

            let mut cmd = Command::new(replay_target_path.as_os_str());
            println!("cmd: {:?}", cmd);
            let exit_status = cmd
